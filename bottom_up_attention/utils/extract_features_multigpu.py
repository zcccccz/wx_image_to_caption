...
from bottom_up_attention.utils.utils import mkdir, save_features
from bottom_up_attention.utils.extract_utils import get_image_blob, save_bbox, save_roi_features_by_bbox, save_roi_features
from bottom_up_attention.utils.progress_bar import ProgressBar
from bottom_up_attention.bua import add_config
from bottom_up_attention.bua.caffe.modeling.box_regression import BUABoxes
...
def extract_feat_multigpu_start(args,cfg):
    ...
    #     extract_feat(i, img_lists[i], cfg, args)

    extract_feat_list = []
    for i in range(num_gpus):
        extract_feat_list.append(extract_feat_multigpu.remote(i, img_lists[i], cfg, args, actor))
